...
export function initializeFCM(): Messaging | null {
  try {
    ...
    // CRITICAL FIX: Gracefully handle missing configuration instead of crashing
    if (!firebaseConfig.projectId) {
      console.warn('[FCM] Firebase projectId is missing. Skipping messaging initialization.');
      return null;
    }

    messaging = getMessaging(app);

    // FIX: Check if the service worker is already registered before trying again
    // This prevents the "fighting" between multiple workers that causes refreshes
    navigator.serviceWorker.getRegistration('/').then((existingReg) => {
      if (!existingReg || existingReg.active?.scriptURL.includes('sw.js')) {
        console.log('[FCM] Registering dedicated messaging worker...');
 ...
 */
export function listenForMessages(callback: (payload: unknown) => void): () => void {
