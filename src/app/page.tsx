...
interface Schedule { id: string; companyId: string; busId: string; routeId: string; departureTime: Date; arrivalTime: Date; price: number; availableSeats: number; status: string }
...
const StatSkeleton = () => <div className="h-20 bg-gray-100 rounded-xl animate-pulse" />;
const CardSkeleton = () => <div className="h-48 bg-gray-100 rounded-2xl animate-pulse" />;

const cache = {
  data: new Map<string, { value: any; exp: number }>(),
  set: (k: string, v: any, ttl: number) => cache.data.set(k, { value: v, exp: Date.now() + ttl }),
...
export default function HomePage() {
  ...
  const fetchSchedules = useCallback(async (refresh = false) => {
    ...
    // Use cache if not refreshing and within 1 minute
    if (!refresh && now - lastFetchRef.current < 60000) {
    ...
    try {
      // Fetch from new Prisma-backed API
      ...
      // Data comes pre-enhanced from API, just cast it
      const enhanced = data as EnhancedSchedule[];
      cache.set(cacheKey, enhanced, 180000); // Cache for 3 minutes
      setSchedules(enhanced);
      setError("");
      lastFetchRef.current = now;
    } catch (err: unknown) {
      console.error('Failed to fetch schedules:', err);
      setError("Failed to load schedules. Check your connection.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
