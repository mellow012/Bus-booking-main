...
interface FormData {
  ...
  firstName: string; lastName: string; agreeToTerms: boolean;
}
interface FormErrors {
  email?: string; password?: string; confirmPassword?: string;
  firstName?: string; lastName?: string; agreeToTerms?: string; general?: string;
}
interface PasswordStrength { score: number; label: string; color: string; suggestions: string[] }

const MIN_PASSWORD_LENGTH = 8;
const MAX_PASSWORD_LENGTH = 128;
const NAME_REGEX = /^[a-zA-Z\s'-]+$/;

// Password strength checker — labels now come from translations
const checkPasswordStrengthRaw = (password: string): { score: number; suggestions: ('upper' | 'number' | 'special')[] } => {
  let score = 0;
  const suggestions: ('upper' | 'number' | 'special')[] = [];
  if (password.length >= MIN_PASSWORD_LENGTH) score++;
  if (password.length >= 12) score++;
  if (/[a-z]/.test(password) && /[A-Z]/.test(password)) { score++; } else { suggestions.push('upper'); }
  if (/\d/.test(password)) { score++; } else { suggestions.push('number'); }
  if (/[!@#$%^&*(),.?":{}|<>]/.test(password)) { score++; } else { suggestions.push('special'); }
...
const useFormValidation = (formData: FormData, t: (key: string, opts?: any) => string) => {
  ...
  const validateField = (name: string, value: string | boolean): string => {
    switch (name) {
      ...
      case 'password':
        ...
        if ((value as string).length < MIN_PASSWORD_LENGTH) return t('errorPasswordMin', { min: MIN_PASSWORD_LENGTH });
        if ((value as string).length > MAX_PASSWORD_LENGTH) return t('errorPasswordMax', { max: MAX_PASSWORD_LENGTH });
        if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(value as string)) return t('errorPasswordStrength');
        return '';
  ...
  const clearFieldError = (name: string) => setErrors(prev => { const n = { ...prev }; delete n[name as keyof FormErrors]; return n; });
...
export default function Register() {
  ...
  const [formData, setFormData] = useState<FormData>({
    email: '', password: '', confirmPassword: '', firstName: '', lastName: '', agreeToTerms: false,
  });
  ...
  const [strengthData, setStrengthData] = useState<{ score: number; suggestions: ('upper' | 'number' | 'special')[] } | null>(null);

  const { errors, touched, validateForm, handleBlur, clearFieldError } = useFormValidation(formData, t);
  const { signUp } = useAuth();
  const { sendVerificationEmail } = useEmailVerification();

  const strengthLabels = ['strengthVeryWeak','strengthWeak','strengthFair','strengthGood','strengthStrong','strengthVeryStrong'] as const;
  ...
  const getErrorMessage = (error: any): string => {
    if (error?.code) {
      switch (error.code) {
        ...
        default:                               return t('authGeneral');
      }
    }
    return t('authUnexpected');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateForm()) { const f = Object.keys(errors)[0]; document.getElementById(f)?.focus(); return; }
    setIsSubmitting(true); setGeneralError('');
    try {
      await signUp(formData.email, formData.password, { firstName: formData.firstName.trim(), lastName: formData.lastName.trim() });
      setUserEmail(formData.email); setSuccess(true);
      try { await sendVerificationEmail(); } catch (e) { console.error('Failed to send verification email:', e); }
  ...
  const renderPasswordStrength = () => {
    ...
    const scoreColor = score >= 4 ? 'text-green-600' : score >= 3 ? 'text-blue-600' : score >= 2 ? 'text-yellow-600' : 'text-red-600';
    ...
    return (
      <div className="mt-2">
        ...
        <div className="flex space-x-1">
          {[...Array(5)].map((_, i) => (
            <div key={i} className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${i < score ? color : 'bg-gray-200'}`} />
          ))}
        </div>
        {suggestions.length > 0 && (
          <ul className="mt-2 space-y-1">
            {suggestions.map((s, i) => {
              const label = s === 'upper' ? t('suggestionUpperLower') : s === 'number' ? t('suggestionNumber') : t('suggestionSpecial');
              return <li key={i} className="text-xs text-gray-600 flex items-start"><span className="mr-1">•</span>{label}</li>;
            })}
          </ul>
        )}
      </div>
          ...
          <div className="bg-white py-10 px-6 shadow-xl rounded-2xl sm:px-12">
            <form className="space-y-5" onSubmit={handleSubmit} noValidate>
              ...
              {generalError && (
                <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg flex items-start" role="alert">
                  ...
                  <div><p className="font-medium">{t('errorTitle')}</p><p className="text-sm mt-1">{generalError}</p></div>
                </div>
              )}
              ...
              {success && (
                <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg flex items-start">
                  ...
                  <div><p className="font-medium">{t('successTitle')}</p><p className="text-sm mt-1">{t('successRedirect')}</p></div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input id="firstName" name="firstName" type="text" autoComplete="given-name" required
                      ...
                      value={formData.firstName} onChange={handleInputChange} onBlur={() => handleBlur('firstName')}
                      className={getInputClassName('firstName')} placeholder={t('firstNamePlaceholder')}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input id="lastName" name="lastName" type="text" autoComplete="family-name" required
                      ...
                      value={formData.lastName} onChange={handleInputChange} onBlur={() => handleBlur('lastName')}
                      className={getInputClassName('lastName')} placeholder={t('lastNamePlaceholder')}
                      disabled={isSubmitting || success} />
                    <UserIcon className="w-5 h-5 text-gray-400 absolute top-2.5 left-3 pointer-events-none" />
                  </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPassword(!showPassword)}
                    ...
                    aria-label={showPassword ? 'Hide password' : 'Show password'} disabled={isSubmitting || success}>
              ...
              <div>
                ...
                <div className="relative">
                  <input id="confirmPassword" name="confirmPassword"
                    ...
                    value={formData.confirmPassword} onChange={handleInputChange} onBlur={() => handleBlur('confirmPassword')}
                    className={getInputClassName('confirmPassword')} placeholder={t('confirmPasswordPlaceholder')}
                    disabled={isSubmitting || success} />
                  <LockClosedIcon className="w-5 h-5 text-gray-400 absolute top-2.5 left-3 pointer-events-none" />
              ...
              <div>
                <div className="flex items-start">
                  <input id="agreeToTerms" name="agreeToTerms" type="checkbox" checked={formData.agreeToTerms}
                    onChange={handleInputChange} onBlur={() => handleBlur('agreeToTerms')}
                  ...
                  <label htmlFor="agreeToTerms" className="ml-2 block text-sm text-gray-700">
                    {t('agreeTermsPre')}{' '}
                    <Link href="/terms" className="text-blue-600 hover:text-blue-500 focus:outline-none focus:underline" target="_blank">
                      {t('termsLink')}
                    </Link>
                    {' '}{t('andWord')}{' '}
                    <Link href="/privacy" className="text-blue-600 hover:text-blue-500 focus:outline-none focus:underline" target="_blank">
                      {t('privacyLink')}
                    </Link>
                  </label>
                </div>
            ...
            <div className="mt-6 text-center">
              <p className="text-xs text-gray-500">
                {t('alreadyHaveAccount')}{' '}
                <Link href="/login" className="text-blue-600 hover:text-blue-500 focus:outline-none focus:underline transition-colors duration-200">
                  {t('signInHere')}
                </Link>
              </p>
            </div>
