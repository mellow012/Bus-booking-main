...
const useFormValidation = (formData: FormData, t: (key: string, opts?: any) => string) => {
  ...
  // Re-validate confirmPassword when password changes if touched
  useEffect(() => {
  ...
  const validateForm = () => {
    ...
    // Return both validation result and calculated errors to prevent stale state issues
    return {
...
// ─── Internal Register Content Component ──────────────────────────────────────
...
function RegisterContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { isValid, errors: currentErrors } = validateForm();
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push(`/verify-email${dest ? `?redirect=${encodeURIComponent(dest)}` : ''}`), 2000);
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col lg:flex-row bg-white overflow-y-auto">
        ...
        <div className="hidden lg:flex lg:w-1/2 xl:w-[55%] relative bg-white items-center justify-center overflow-hidden">
          <div className="relative z-10 flex flex-col items-center px-12 py-16 max-w-lg text-center">
            <Image
              ...
              priority
            />
            <h2 className="mt-6 text-2xl xl:text-3xl font-extrabold text-gray-900 tracking-tight">
              Join TibhukeBus today
            </h2>
            <p className="mt-3 text-sm xl:text-base text-gray-500 leading-relaxed">
              Create an account to book tickets, manage reservations, and track your journeys live.
            </p>
            <StorysetCredit
              href="https://storyset.com/phone"
              label="Phone illustrations by Storyset"
              placement="bottom"
              className="mt-4"
            />
          </div>
        ...
        {/* ── Right panel: form ── */}
        <div className="flex-1 flex flex-col items-center justify-start pt-36 sm:pt-40 lg:pt-20 lg:justify-center pb-8 sm:px-6 lg:px-8">
            ...
            <div className="bg-white py-10 px-6 shadow-xl rounded-2xl sm:px-12">
                ...
                <div>
                  <div className="flex items-start">
                    <input
                      ...
                      className="h-4 w-4 mt-0.5 accent-brand-700 focus:ring-brand-700 border-gray-300 rounded transition-colors duration-200"
                      disabled={isSubmitting || success}
              ...
              <div className="mt-6">
                ...
                <div className="mt-6">
                  <Button
                    ...
                    variant="outline"
                    className="w-full flex justify-center items-center py-2.5 px-4 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-700 transition-all duration-200"
                  >
                    {isGoogleLoading ? (
              ...
              <div className="mt-6 text-center">
                <p className="text-xs text-gray-500">
                  ...
                  <Link href={`/login${dest ? `?redirect=${encodeURIComponent(dest)}` : ''}`}
...
// ─── Default Page Export (wrapped safely in Suspense) ──────────────────────────
