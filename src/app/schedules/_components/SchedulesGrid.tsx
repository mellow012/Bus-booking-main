...
export default function SchedulesGrid({ regularSchedules, selectedCategory, setSelectedCategory, handleBooking, userCity, sortBy }: any) {
  // If user selected explicit sort (price, seats, company) or selected a single time category, render continuous sorted grid
  if (sortBy === 'price' || sortBy === 'price_desc' || sortBy === 'seats' || sortBy === 'company' || selectedCategory) {
    const items = selectedCategory
      ? regularSchedules.filter((s: any) => getScheduleCategory(s as any) === selectedCategory)
      : regularSchedules;
      ...
      <div className="space-y-6">
        {selectedCategory && (
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-black text-gray-900 tracking-tight">{selectedCategory}</h3>
  ...
  // When sortBy === 'time' (default time view): Group neatly by time category
  return (
    <>
      {categories.map((cat) => {
