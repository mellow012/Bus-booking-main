...
import { setUserOperator } from '@/lib/actions/user.actions';
...
export async function POST(req: Request, context: any) {
  ...
  try {
    const res = await setUserOperator(targetId, { id: current.id, name: `${current.firstName || ''} ${current.lastName || ''}`.trim(), role: current.role, companyId: current.companyId ?? undefined });
    ...
    console.error('Error in set-operator route:', err);
