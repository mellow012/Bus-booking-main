...
export default function OverviewTab({ dashboard }: OverviewTabProps) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500">Filter recent bookings by booking date.</p>
          </div>
          <div className="flex items-center gap-2">
            <label htmlFor="admin-booking-date-filter" className="sr-only">Filter by booking date</label>
            <input
            ...
            {bookingFilterDate && (
              <button
                ...
                className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-100"
              >
                Clear
              </button>
            )}
        ...
        {filteredBookings.length === 0 ? (
          <div className="py-8 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200">
            ...
            <p className="text-gray-500">No bookings match the selected date.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {groupedBookings.map(({ date, label, items }) => (
              <div key={date} className="space-y-3">
                <div className="flex items-center justify-between rounded-2xl bg-gray-50 px-4 py-3 border border-gray-200">
                  <div>
                    <p className="text-sm font-semibold text-gray-700">{label}</p>
                    <p className="text-xs text-gray-500">{items.length} booking{items.length !== 1 ? 's' : ''}</p>
                  </div>
                </div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {items.map((b: Booking) => (
                        <tr key={b.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(b.createdAt).toLocaleTimeString([], { hour:'2-digit', minute:'2-digit' })}</td>
      ...
      {selectedBooking && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={closeBooking}>
          <div className="w-full max-w-3xl rounded-3xl bg-white border border-gray-200 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              ...
              <button onClick={closeBooking} className="rounded-full p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="grid gap-4 p-6 md:grid-cols-2 max-h-[50vh] overflow-y-auto">
              <div className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="rounded-3xl border border-gray-100 bg-gray-50 p-4">
                  ...
                  <p className="mt-3 text-sm text-gray-500">Amount: MWK {selectedBooking.totalAmount?.toLocaleString()}</p>
                </div>
                <div className="rounded-3xl border border-gray-100 bg-gray-50 p-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-gray-500">Passenger details</p>
                  <div className="mt-3 space-y-3 text-sm text-gray-700">
                    {selectedBooking.passengerDetails?.map((passenger: any, index: number) => (
            ...
            <div className="flex flex-col gap-3 p-6 border-t border-gray-100 bg-gray-50 md:flex-row md:items-center md:justify-between">
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-500">Respond to booking request or remind customer.</p>
              </div>
