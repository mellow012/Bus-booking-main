...
export default function OperatorsAndBranchesTab({ dashboard }: OperatorsAndBranchesTabProps) {
  ...
  const { data: rawOperators = [] } = useCompanyOperators(companyId);
  const { data: rawBranches = [] } = useCompanyRegions(companyId);
  const { data: availableRoutes = [] } = useCompanyRoutes(companyId);
  ...
  const operators = useMemo<OperatorRow[]>(() => {
    return rawOperators.map((operator: any) => {
      ...
      // regionId is now reliably sourced from the Operator table via useCompanyOperators
      const operatorRegionId = operator.regionId || null;
      
      // User.region is a plain string (region name) stored when assignment is saved
      const operatorRegionName = typeof operator.region === 'string' ? operator.region : '';
      
      // First: try to match branch by id (most reliable)
      const matchingBranch = operatorRegionId
        ? rawBranches.find((branch: any) => branch.id === operatorRegionId)
        // Second: try to match by name string (legacy fallback)
        : (operatorRegionName
          ? rawBranches.find((branch: any) => branch.name?.toLowerCase() === operatorRegionName.toLowerCase())
          : undefined);
      
      const branchName = matchingBranch?.name || operatorRegionName || 'Unassigned';
      const branchId = matchingBranch?.id || operatorRegionId || null;
      ...
      return {
        ...
        status: operator.operatorStatus || operator.status || (operator.isActive === false ? 'inactive' : 'active'),
        region: branchName,
        regionId: branchId,
        routeIds: existingRouteIds,
      };
    });
  }, [rawOperators, rawBranches]);
