...
export default function AdminDashboard() {
  ...
  // Update dashboard data when bookings change
  useEffect(() => {
  ...
  const recentPayments = useMemo(() => {
    return bookings
      .filter((b) => b.paymentStatus === "paid")
      .sort((a, b) => convertFirestoreDate(b.updatedAt).getTime() - convertFirestoreDate(a.updatedAt).getTime())
      .slice(0, 5);
  }, [bookings]);
                    ...
                    <div key={booking.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">
                          {formatRelativeTime(convertFirestoreDate(booking.updatedAt))}
                        </p>
                      </div>
                      <div className="text-right">
          ...
          <div className="bg-white rounded-xl p-6 border shadow-sm">
            ...
            <div className="space-y-3">
              {recentCancellations.length === 0 ? (
                <p className="text-gray-500 text-sm text-center py-4">No recent cancellations</p>
              ) : (
        ...
        <div className="bg-white rounded-xl p-6 border shadow-sm">
          ...
          <div className="h-64 bg-gray-50 rounded-lg flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400">Revenue: MWK {statistics.thisMonthRevenue.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
