...
export function useOperatorDashboard() {
  ...
  const fetchInitialData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (opData) {
        // Resolve region name from Region table if regionId is set
        let enrichedOpData: any = { ...opData };
        if (opData.regionId) {
          const { data: regionData } = await supabase
            .from('Region')
            .select('id, name')
            .eq('id', opData.regionId)
            .maybeSingle();
          if (regionData) {
            enrichedOpData = { ...enrichedOpData, region: regionData.name, regionName: regionData.name };
          }
        }
        setOperatorInfo(enrichedOpData);
      ...
      // 5.5 Fetch recurring templates scoped to these routes
  ...
  }, [user?.id, companyId, authLoading, userProfile?.role, operatorIdParam]);
  ...
  useEffect(() => {
    ...
    fetchInitialData();
  }, [user?.id, userProfile?.role, authLoading, router, fetchInitialData]);
  ...
  return {
    ...
    setSuccessMessage,
    operatorInfo,
    companyName: operatorInfo?.companyName || (userProfile as any)?.companyName,
    companyLogo,
    assignedRoutes,
    schedules,
    templates,
