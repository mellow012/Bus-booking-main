...
export default function OperatorDashboard() {
  ...
  try {
    ...
    // 2. Format the data for Firestore
    const firestoreData = {
      ...data,
      // Convert JS Dates to Firestore Timestamps for consistency
      ...
      // Use the clean companyId from your dashboard state
      companyId: companyId, 
      createdBy: user?.uid
    };
    ...
    // 4. Return the ID so the handleAdd in SchedulesTab can update the local state
    return docRef.id;
  } catch (error: any) {
    console.error("Error in addSchedule:", error);
    showAlert("error", `Failed to save schedule: ${error.message}`);
    throw error; // Throwing ensures the modal stays open so the user doesn't lose data
  }
}}
            {...commonProps}
          />
