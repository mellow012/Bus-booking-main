...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('Bookings page error:', error, errorInfo);
  }
...
const BookingCard = memo<{
  ...
  ({
    ...
    const handleCancel = useCallback(() => onCancel(booking.id, booking.scheduleId, booking.seatNumbers), [
      booking.id,
      booking.scheduleId,
      booking.seatNumbers,
      onCancel,
    ...
    // Show segment stops if available
    const originName =
      booking.route?.stops?.find((s) => s.id === booking.originStopId)?.name || booking.route?.origin || 'N/A';
    const alightName =
      booking.route?.stops?.find((s) => s.id === booking.destinationStopId)?.name ||
        ...
        <div className="p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-gray-600 truncate">
                  Ref: {booking.bookingReference || booking.id.slice(-8)}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium border ${getPaymentStatusColor(booking.paymentStatus)}`}
              >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 p-3 bg-gray-50 rounded-xl mb-4">
            ...
            <div className="text-center min-w-[80px]">
              ...
              <div className="text-sm text-gray-600 flex items-center justify-center gap-1">
                <MapPin className="w-3 h-3" />
                <span className="truncate">{alightName}</span>
              </div>
            ...
            <div className="md:col-span-2 lg:col-span-2">
              <div className="grid grid-cols-2 gap-3 text-sm text-gray-600">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {booking.passengerDetails?.length || 0} passenger
                    {(booking.passengerDetails?.length || 0) > 1 ? 's' : ''}
                  </span>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col justify-between lg:col-span-1">
              ...
              {booking.paymentProvider && (
                <div className="text-right text-xs text-gray-500 mt-1 mb-3 flex items-center gap-2 justify-end">
                  <img
                    src={getPaymentMethodIcon(booking.paymentProvider).logo}
                    alt={`${getPaymentMethodIcon(booking.paymentProvider).label} logo`}
                    className="w-6 h-6"
                  />
...
const BookingsPage: React.FC = () => {
    ...
    switch (method) {
      case 'mobile_money':
        return {
          ...
          logo: '/Airtel Money Uganda Logo PNG Vector (PDF) Free Download.JPG0',
        };
      case 'card':
      case 'credit_card':
        return {
  ...
  function isBookingExpired(booking: BookingWithDetails): boolean {
    const now = new Date();
    const arrivalDate =
      booking.schedule.arrivalDateTime instanceof Timestamp
  ...
  // ✅ CORE FIX: Apply filters logic as a standalone function
  // This is NOT a useCallback - it's a regular function that will be called from effects
  const applyFiltersLogic = (
    bookingsToFilter: BookingWithDetails[],
    ...
    if (currentFilters.busType) {
      const busTypesToFilter = Array.isArray(currentFilters.busType) ? currentFilters.busType : [currentFilters.busType];
    ...
    if (currentFilters.company) {
      filtered = filtered.filter((b) => b.company?.name === currentFilters.company);
    }
  ...
  const fetchBookings = useCallback(async (retryCount = 0) => {
    ...
    try {
      ...
      // ✅ Call applyFiltersLogic with current filter state
    ...
    try {
      ...
      if (!isCancellationRequest) {
        ...
        sendNotification({
          ...
          ...NotificationTemplates.bookingConfirmed(booking.id, `${booking.route.origin} → ${booking.route.destination}`),
  ...
  const handleStatusFilter = useCallback((status: string) => {
    ...
    // Immediately apply filters with new status
  ...
  // ✅ FIX: Initial load effect - minimal dependencies
  useEffect(() => {
    if (!user) {
      router.push('/login');
      return;
  ...
  // ✅ FIX: Apply filters effect - only triggers on filter/status changes
  useEffect(() => {
    applyFiltersLogic(bookings, activeFilter, filters);
  }, [activeFilter, filters, bookings]);
