...
export const ScheduleCard = React.memo(({ s, onBook, userCity }: {
  ...
  React.useEffect(() => {
    console.log(`[ScheduleCard] Route: ${s.origin} → ${s.destination}, Distance: ${s.distance}`);
  }, [s.id, s.distance, s.origin, s.destination]);

  return (
    <article className="group bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-brand-200 transition-all duration-300 overflow-hidden flex flex-col">
      <div className={`h-[3px] w-full ${filling ? "bg-gradient-to-r from-rose-400 to-orange-400" : isLocal ? "bg-gradient-to-r from-teal-400 to-emerald-500" : "bg-gradient-to-r from-brand-600 to-brand-700"}`} />
        ...
        <div className="flex items-start justify-between gap-2 sm:gap-3 mb-3 sm:mb-4">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            {s.companyLogo ? (
              ...
              <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-br from-brand-600 to-brand-700 flex items-center justify-center text-white font-bold text-sm sm:text-base shrink-0">
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-xl p-2.5 sm:p-3 mb-3">
          <div className="flex items-center">
            <div className="text-center min-w-0 flex-1">
              ...
              <p className="text-[10px] sm:text-[11px] text-brand-700 font-semibold mt-0.5">{s.departureTime}</p>
            </div>
            <div className="flex flex-col items-center gap-0.5 px-1 sm:px-3 shrink-0">
              <div className="flex items-center gap-0.5">
                <div className="w-1.5 h-1.5 rounded-full bg-brand-400" />
                <div className="w-3 sm:w-7 h-px bg-brand-200" />
                <ArrowRight className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-brand-700" />
                <div className="w-3 sm:w-7 h-px bg-brand-200" />
                <div className="w-1.5 h-1.5 rounded-full bg-brand-400" />
              </div>
            ...
            <div className="text-center min-w-0 flex-1">
              ...
              <p className="text-[10px] sm:text-[11px] text-brand-700 font-semibold mt-0.5">{s.arrivalTime}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1 mb-3 min-h-[18px]">
          {category && (
            <span className={`flex items-center gap-0.5 sm:gap-1 text-[10px] sm:text-[11px] px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full border font-semibold ${category === 'Boarding Now' ? 'bg-orange-50 text-orange-700 border-orange-200 animate-pulse' :
                category === 'Morning' ? 'bg-brand-50 text-brand-700 border-brand-200' :
