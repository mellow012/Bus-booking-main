    ...
    <div className="space-y-6 sm:space-y-8 px-2 sm:px-0">
          ...
          {filteredRoutes.map(route => (
            <div key={route.id} className="bg-white rounded-2xl sm:rounded-2xl border border-gray-100 overflow-hidden flex flex-col hover:shadow-md transition-all duration-500 group relative text-left">
              <div className="absolute top-6 right-6">
                <span className={`px-3 py-1.5 text-[9px] font-bold rounded-xl uppercase tracking-widest border shadow-sm ${
              ...
              <div className="p-6 sm:p-8 flex-1 flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-6 mb-8">
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Starting Yield</p>
                    <p className="text-lg font-bold text-gray-900 tracking-tight">MWK {fmt(route.baseFare || 0)}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Est. Duration</p>
                    <p className="text-lg font-bold text-gray-900 tracking-tight">{route.duration}m</p>
                  </div>
                </div>
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Define New Corridor">
        <form onSubmit={handleAdd} className="space-y-4 sm:space-y-6 text-left p-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Origin Hub</label>
              <input
                type="text"
          ...
          <div className="flex justify-end gap-3 pt-8 border-t border-gray-50">
            <button type="button" onClick={() => setShowAddModal(false)} className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-gray-600 transition-colors">Cancel</button>
            <button
              type="submit"
      ...
      <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Update Corridor Architecture">
        <form onSubmit={handleEdit} className="space-y-4 sm:space-y-6 text-left p-1">
          ...
          <div className="flex justify-end gap-3 pt-8 border-t border-gray-50">
            <button type="button" onClick={() => setShowEditModal(false)} className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-gray-600 transition-colors">Discard</button>
            <button
              type="submit"
              disabled={actionLoading}
              className="bg-indigo-600 text-white px-8 py-4 rounded-2xl text-[11px] font-bold uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"
