  ...
  return (
    <section id="schedules-section" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-brand-700 mb-1">Live Availability</p>
        ...
        <button onClick={()=>router.push("/schedules")} className="group flex items-center gap-2 text-sm font-semibold text-brand-700 hover:text-brand-800 transition-colors">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <button onClick={() => setShowSort(s => !s)} className="flex items-center gap-2 px-3.5 py-2 text-sm text-gray-600 bg-white border border-gray-200 rounded-xl hover:border-brand-300 transition-all shadow-sm">
                ...
                {showSort && (
                  <div className="absolute left-0 top-full mt-1.5 bg-white border border-gray-200 rounded-xl shadow-xl z-30 py-1.5 min-w-[180px]">
                    {SORT_OPTIONS.map(o => (
                      <button key={o.value} onClick={() => { setSortKey(o.value); setShowSort(false); }} className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${sortKey === o.value ? "bg-brand-50 text-brand-700 font-semibold" : "text-gray-700 hover:bg-gray-50"}`}>
                        {o.label}
                      </button>
                    ))}
                  </div>
                ...
                return (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all active:scale-95 whitespace-nowrap ${
                      isSelected
                        ? "bg-brand-700 text-white shadow-md shadow-brand-100"
      ...
      {!loading && totalPages > 1 && (
        <div className="flex items-center justify-center gap-4 mt-8 pb-4">
          <button 
            ...
            className="p-2 rounded-xl border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 hover:border-brand-300 disabled:opacity-40 disabled:hover:bg-white disabled:hover:border-gray-200 transition-all shadow-sm active:scale-95"
              ...
              return (
                <button
                  ...
                  className={`w-10 h-10 rounded-xl text-sm font-bold transition-all ${
                    currentPage === page 
                      ? "bg-brand-700 text-white shadow-md shadow-brand-100 scale-105" 
                      : "bg-white border border-gray-100 text-gray-400 hover:border-brand-200 hover:text-brand-650"
                  }`}
                >
                  {page}
                </button>
