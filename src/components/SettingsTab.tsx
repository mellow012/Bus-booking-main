...
const PaymentSettings: React.FC<{
  ...
  const { register, handleSubmit, reset, watch } = useForm({
    defaultValues: {
      ...
      returnDiscount:         company?.paymentSettings?.returnDiscount         ?? 0,
    },
  });
  ...
  const onSubmit = async (data: {
    ...
    try {
      ...
      const updatedPaymentSettings = {
        ...
        returnDiscount:         Number(data.returnDiscount) || 0,
      };
        ...
        {!company.paymentSettings?.paychanguEnabled ? (
          ...
          <div className="rounded-xl border border-gray-100 overflow-hidden shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-gray-100">
              ...
              <div className="bg-white px-5 py-3.5">
                <p className="text-xs text-gray-400 mb-0.5">Return Trip Discount</p>
                <p className="text-sm font-bold text-emerald-600">{company.paymentSettings.returnDiscount || 0}% Discount</p>
              </div>
            </div>
          </div>
        )}
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="max-w-xs">
            <Label htmlFor="returnDiscount" className="text-xs font-medium text-gray-700 mb-1.5 block">Return Trip Discount (%)</Label>
            <Input id="returnDiscount" type="number" min="0" max="100" {...register("returnDiscount")} placeholder="10" className="h-9 text-sm" />
          </div>
        </div>
