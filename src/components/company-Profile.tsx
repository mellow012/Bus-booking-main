...
import {
  ...
  Clock, MessageCircle, Calendar, ChevronRight,
  Navigation, FileText, MapPin, Plus, X
} from "lucide-react";
...
// FIX C-1: handles legacy comma-string OR native array — normalises to string[]
...
const CompanyProfileTab: FC<CompanyProfileTabProps> = ({
  ...
  // FIX C-2: track raw File for upload; preview is a separate object URL
  ...
  // FIX C-1: always keep branches as string[] in state
  ...
  const todaysSchedules = (schedules || []).filter(s => {
    const d = s.departureDateTime instanceof Date ? s.departureDateTime : s.departureDateTime?.toDate?.() || null;
  ...
  const handleAddBranch = () => {
    ...
    if (!t)                { setError("Branch name cannot be empty");   return; }
    if (branches.includes(t)) { setError("This branch already exists"); return; }
  ...
  // FIX C-2: upload logo to Firebase Storage, return persistent download URL
  const uploadLogo = async (companyId: string): Promise<string | null> => {
    if (!logoFile) return null;
    const ext  = logoFile.name.split(".").pop() || "jpg";
    const sRef = storageRef(storage, `company-logos/${companyId}/logo.${ext}`);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIX C-2: upload new logo if selected, otherwise keep existing URL
      let logoUrl = editData.logo || null;
      if (logoFile) logoUrl = await uploadLogo(editData.id);

      // FIX C-1: branches stored as string[] (array) — never comma-string
      // FIX C-3: use branches state explicitly so it can't diverge from editData
      const payload = {
        ...editData,
        logo:           logoUrl,
        branches:       branches,
      ...
      await updateDoc(doc(db, "companies", editData.id), payload);

      if (isInitialSetup)
        await updateDoc(doc(db, "users", userProfile.id), { setupCompleted: true, updatedAt: now });
      ...
      if (isInitialSetup) { await refreshUserProfile(); router.push("/company/admin"); }
    } catch (err: any) {
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          <div className="relative group">
            <div className="w-32 h-32 rounded-3xl bg-white/20 backdrop-blur-xl p-1.5 border border-white/30 shadow-inner">
              <img
                ...
                className="w-full h-full rounded-[1.25rem] object-cover shadow-sm" alt="Company Logo"
              />
            </div>
            {isEditing && (
              <label className="absolute -bottom-2 -right-2 bg-white text-blue-600 p-2.5 rounded-2xl shadow-xl cursor-pointer hover:scale-110 transition-transform border border-gray-100">
                <Camera size={20} />
                {/* FIX C-2: store File + create object URL preview */}
          ...
          <div className="text-center md:text-left flex-1">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3">
              <span className="bg-white/20 backdrop-blur-md px-4 py-1 rounded-full text-sm font-bold border border-white/10">{company.status || "Active"}</span>
              <span className="bg-blue-400/30 backdrop-blur-md px-4 py-1 rounded-full text-sm font-bold border border-white/10">{branches.length} {branches.length === 1 ? "Branch" : "Branches"}</span>
              <span className="bg-blue-400/30 backdrop-blur-md px-4 py-1 rounded-full text-sm font-bold border border-white/10">{routes.length} Active Routes</span>
            </div>
          </div>
          {!isEditing && (
            <button onClick={() => setIsEditing(true)} className="bg-white text-blue-700 px-8 py-4 rounded-2xl font-black shadow-lg hover:bg-blue-50 transition-all active:scale-95 flex items-center gap-2">
      ...
      {isEditing ? (
        <form onSubmit={handleUpdate} className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 space-y-6">
            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3"><Building2 className="text-blue-500" size={28} /> General Info</h2>
            <div className="space-y-4">
              {[
                { label: "Phone Number",     key: "contact",     type: "input"    },
                { label: "Physical Address", key: "address",     type: "input"    },
                { label: "Bio",              key: "description", type: "textarea" },
              ].map(({ label, key, type }) => (
                <div key={key}>
                  <label className="text-xs font-black uppercase tracking-widest text-gray-400 ml-1">{label}</label>
                  {type === "textarea" ? (
                    <textarea className="w-full p-4 bg-gray-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl transition-all outline-none h-40 resize-none mt-1"
                      value={(editData as any)?.[key] || ""}
                      onChange={e => setEditData(prev => prev ? { ...prev, [key]: e.target.value } : null)} />
                  ) : (
                    <input className="w-full p-4 bg-gray-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl transition-all outline-none mt-1"
                      value={(editData as any)?.[key] || ""}
                      onChange={e => setEditData(prev => prev ? { ...prev, [key]: e.target.value } : null)} />
                  )}
                </div>
          ...
          <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3 mb-6"><MapPin className="text-green-500" size={28} /> Operating Branches</h2>
            <div className="space-y-4 mb-6">
              <p className="text-sm text-gray-600">Add all branches/regions where your company operates. These are used to match operators to routes.</p>
              <div className="flex gap-2">
                ...
                <Button type="button" onClick={handleAddBranch} className="px-6 bg-green-600 hover:bg-green-700 text-white rounded-xl flex items-center gap-2">
                  <Plus size={18} /> Add
                </Button>
              </div>
            </div>
            {branches.length > 0 ? (
              <div className="space-y-2">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Active Branches ({branches.length})</p>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {branches.map((branch, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-green-50 border border-green-100 rounded-xl group hover:bg-green-100 transition-colors">
                      <div className="flex items-center gap-2"><MapPin size={16} className="text-green-600" /><span className="font-semibold text-gray-900">{branch}</span></div>
            ...
            <div className="mt-4 p-4 bg-yellow-50 rounded-xl border border-yellow-100">
              <p className="text-xs text-yellow-800"><strong>Note:</strong> Operators' region must exactly match one of these branch names to be assignable to routes.</p>
            </div>
          </div>

          {/* Operating hours */}
          <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 lg:col-span-2">
            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3 mb-8"><Clock className="text-blue-500" size={28} /> Operating Hours</h2>
            <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      {!d.closed ? (
                        <div className="flex items-center gap-2">
                          <input type="time" value={d.open}  onChange={e => handleHoursChange(day, "open",  e.target.value)} className="p-2 border rounded-xl" />
                          <span className="text-gray-300">-</span>
                          <input type="time" value={d.close} onChange={e => handleHoursChange(day, "close", e.target.value)} className="p-2 border rounded-xl" />
                        </div>
                      ) : <span className="text-red-400 font-bold italic">Closed</span>}
                      <button type="button" onClick={() => handleHoursChange(day, "closed", !d.closed)}
                        className={`text-[10px] font-black px-3 py-2 rounded-xl border-2 ${d.closed ? "bg-green-50 text-green-600 border-green-200" : "bg-gray-50 text-gray-400 border-gray-200"}`}>
                        {d.closed ? "OPEN" : "CLOSE"}
                      </button>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-2 flex items-center justify-between bg-blue-50 p-6 rounded-[2rem]">
            <p className="text-blue-700 text-sm font-medium">
              ...
              {logoFile && <span className="ml-2 text-blue-600 font-semibold">New logo ready to upload.</span>}
            </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold flex items-center gap-3"><MapPin className="text-green-600" size={28} /> Our Branches</h3>
                <span className="bg-green-50 text-green-700 px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest">{branches.length} {branches.length === 1 ? "Location" : "Locations"}</span>
              </div>
              {branches.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {branches.map((branch, i) => (
                    <div key={i} className="flex items-center gap-3 p-4 bg-gradient-to-br from-green-50 to-white rounded-2xl border border-green-100">
                      <div className="w-10 h-10 rounded-xl bg-green-100 flex items-center justify-center flex-shrink-0"><MapPin size={20} className="text-green-600" /></div>
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-2 px-3 py-1 bg-red-50 text-red-600 rounded-full animate-pulse">
                  <div className="w-2 h-2 bg-red-600 rounded-full" /><span className="text-xs font-black uppercase">Live Board</span>
                </div>
              </div>
              <div className="space-y-4">
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-2xl font-bold flex items-center gap-3"><Navigation className="text-indigo-600" size={28} /> Service Network</h3>
                <span className="bg-indigo-50 text-indigo-700 px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest">{routes.length} Active Routes</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {routes.length > 0 ? routes.map((route, i) => (
                  <div key={route.id || i} className="relative overflow-hidden p-7 bg-gradient-to-br from-indigo-50/50 to-white rounded-[2rem] border border-indigo-100 hover:shadow-xl transition-all">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col gap-5">
                        <p className="font-black text-gray-900 uppercase">{route.origin || route.departureLocation || "?"}</p>
                        <p className="font-black text-gray-900 uppercase">{route.destination || route.arrivalLocation || "?"}</p>
                      </div>
                    </div>
          ...
          <div className="space-y-8">
            {company.contact && (
              <a href={`https://wa.me/${company.contact.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="block">
                <div className="bg-green-500 rounded-[2rem] p-8 text-white shadow-xl hover:-translate-y-1 transition-all flex items-center gap-6">
                  ...
                  <div><p className="font-black text-2xl">WhatsApp</p><p className="text-sm opacity-80 uppercase font-bold tracking-widest">Direct Line</p></div>
                </div>
              </a>
            )}
            <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
                ...
                {Object.keys(DEFAULT_HOURS).map(day => {
                  ...
                  return (
                    <div key={day} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0">
                      ...
                      <span className={`font-black ${h.closed ? "text-red-400" : "text-gray-900"}`}>{h.closed ? "Closed" : `${h.open} - ${h.close}`}</span>
                    </div>
                  );
                })}
              </div>
