...
import { collection, updateDoc, deleteDoc, doc, getDocs,getDoc, query, where } from 'firebase/firestore';
...
const SchedulesTab: FC<SchedulesTabProps> = ({
  ...
  const stats = useMemo(() => ({
    ...
    upcoming: validSchedules.filter(
      (s) => convertFirestoreDate(s.departureDateTime) > new Date() && s.status === 'active'
    ).length,
  ...
  const handleAdd = useCallback(async (e: React.FormEvent) => {
    ...
    if (userProfile?.role === 'operator') {
      ...
      if (!routeData.assignedOperatorIds?.includes(user?.uid)) {
        setError('You are not assigned to this route');
        return;
      }
    }
    ...
    <div className="space-y-6">
      ...
      {filteredSchedules.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border p-12 text-center">
          ...
          {validSchedules.length === 0 && userProfile?.role === 'operator' && (
            <button
              onClick={() => {
                setNewSchedule(createInitialSchedule(companyId));
                setRecurrenceConfig({ type: 'none', interval: 1, daysOfWeek: [] });
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-x-auto">
          <div className="grid grid-cols-7 text-center font-medium text-gray-700 border-b bg-gray-50 min-w-[1000px]">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d) => (
              <div key={d} className="p-4 border-r last:border-r-0">
                {d}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7 min-w-[1000px]">
            {Array.from({ length: 7 }).map((_, i) => {
              const daySchedules = filteredSchedules
                ...
                .sort(
                  (a, b) =>
                    convertFirestoreDate(a.departureDateTime).getTime() -
                    convertFirestoreDate(b.departureDateTime).getTime()
                );
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="w-20 px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100"
                        />
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100"
                        />
                      </div>
                    </div>
                  </div>
