...
// Accepts any value including DBDate (Date | string), numbers, null/undefined.
// Using `unknown` + explicit guards means TypeScript is happy at every call site regardless
// of how strictly the caller's type is inferred (e.g. DBDate | null).
...
async function archiveSchedule(
  ...
  try {
    // We update the schedule status to 'archived' via updateSchedule action
    const updateResult = await dbActions.updateSchedule(s.id, {
      status: 'archived',
      isActive: false,
    });
    if (!updateResult.success) throw new Error(updateResult.error);
    
    // Note: Reports could be handled by a dedicated report action or just prisma call if exposed
    // For now, we rely on the schedule being updated in the DB.
  } catch (err) {
...
const ScheduleCard: FC<{
  ...
  isAdmin: boolean;
}> = ({ schedule, bucket, route, bus, staffMap, userProfile, onEdit, onStatusChange, onArchiveNow, loading, hoursUntilArchive, isAdmin }) => {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] border-y border-r border-gray-100 border-l-4 hover:shadow-[0_4px_15px_-3px_rgba(0,0,0,0.1)] transition-all duration-200 ${borderCls}`}>
      <div className="p-4">
        {/* Route + status row */}
        ...
        {/* Staff actions (Admins and Operators) */}
        {(userProfile?.role === 'operator' || isAdmin) && (
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (!fetchError && data) {
          const reportsData = data.map(r => ({
            ...r,
            departureDateTime: new Date(r.departureDateTime),
            arrivalDateTime:   new Date(r.arrivalDateTime),
            archivedAt:        new Date(r.archivedAt),
          } as ScheduleReport));
          reportsData.sort((a, b) => b.departureDateTime.getTime() - a.departureDateTime.getTime());
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          <div key={label} className="bg-white p-4 rounded-xl border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">{label}</p>
              <p className={`text-xl font-extrabold ${colour}`}>{value}</p>
            </div>
      ...
      {/* Filter + table */}
      <div className="bg-white rounded-xl border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] overflow-hidden">
...
const SchedulesTab: FC<SchedulesTabProps> = ({
  ...
  companyId, addSchedule, setError, setSuccess, user, userProfile, isAdmin
}) => {
  ...
  const [newTemplate,      setNewTemplate]      = useState<Omit<ScheduleTemplate,'id'>>(() => emptyTemplate(companyId, user?.id ?? ''));
  ...
  const assignedRouteIds = useMemo(() => {
    ...
    routes.forEach(r => { if ((r.assignedOperatorIds ?? []).includes(user?.id)) ids.add(r.id); });
    return ids;
    ...
    (async () => {
      try {
        ...
        if (!fetchError && data) {
          ...
          const getName = (u: any) => u.firstName && u.lastName ? `${u.firstName} ${u.lastName}`.trim() : u.email || '?';
          data.forEach(u => {
            const n = getName(u);
            map.set(u.id, n);
            // Removed legacy uid mapping
          });
          setStaffMap(map);
        }
      } catch (err) {
  ...
  const applyStatus = async (s: Schedule, newStatus: ScheduleStatus | 'archive_now') => {
    ...
    try {
      if (newStatus === 'archive_now') {
        ...
        const result = await dbActions.updateSchedule(s.id, {
          status:          newStatus,
    ...
    try {
      ...
      for (const tpl of active) {
        for (let offset = 0; offset < WINDOW_DAYS; offset++) {
          ...
          await dbActions.createSchedule({
            companyId: tpl.companyId,
            busId: tpl.busId,
            routeId: tpl.routeId,
            departureDateTime: dep,
            arrivalDateTime: arr,
            availableSeats: tpl.availableSeats ?? bus?.capacity ?? 0,
            price: tpl.price,
            status: 'active',
      ...
      // We don't manually update setSchedules here anymore because we'll rely on the main dashboard to refetch/realtime
    } catch (e: any) { setError(`Materialisation failed: ${e.message}`); }
    finally { setMaterLoading(false); }
  }, [templates, busMap, setError, setSuccess]);
  ...
  const handleDeleteTemplate = useCallback(async (id: string) => {
    ...
    setActionLoading(true);
    try {
      const { success, error } = await dbActions.deleteSchedule(id);
  ...
  const handleToggleTemplate = useCallback(async (tpl: ScheduleTemplate) => {
    setActionLoading(true);
    try {
      const { success, error } = await dbActions.updateSchedule(tpl.id, {
        isActive: !tpl.isActive, status: !tpl.isActive ? 'active' : 'inactive', updatedAt: new Date(),
      });
      if (!success) throw new Error(error);
  ...
  const handleEditSchedule = useCallback(async (e: React.FormEvent) => {
    ...
    setActionLoading(true);
    try {
      const result = await dbActions.updateSchedule(editSchedule.id, {
        routeId: editSchedule.routeId, busId: editSchedule.busId,
        departureDateTime: toDate(editSchedule.departureDateTime),
        arrivalDateTime:   toDate(editSchedule.arrivalDateTime),
      ...
      setSchedules(prev => prev.map(s => s.id === editSchedule.id ? result.data as Schedule : s));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {([
          { label: 'Live Now',     value: stats.live,      colour: 'text-green-600',  icon: <Radio          className="w-5 h-5 text-green-600" />,  bgObj: 'bg-green-50', bucket: 'live'     as Bucket },
          { label: 'Today',        value: stats.today,     colour: 'text-blue-600',   icon: <Flame          className="w-5 h-5 text-blue-600" />,   bgObj: 'bg-blue-50', bucket: 'today'    as Bucket },
          { label: 'Upcoming',     value: stats.upcoming,  colour: 'text-slate-600',  icon: <CalendarClock  className="w-5 h-5 text-slate-600" />,  bgObj: 'bg-slate-50', bucket: 'upcoming' as Bucket },
          { label: 'Past 24h',     value: stats.past,      colour: 'text-amber-600',  icon: <Clock3         className="w-5 h-5 text-amber-600" />,  bgObj: 'bg-amber-50', bucket: 'past'     as Bucket },
          { label: 'Templates',    value: stats.templates, colour: 'text-purple-600', icon: <LayoutTemplate className="w-5 h-5 text-purple-600" />, bgObj: 'bg-purple-50', bucket: null },
          { label: 'Avail Seats',  value: stats.totalSeats,colour: 'text-gray-800',   icon: <Users          className="w-5 h-5 text-gray-600" />,   bgObj: 'bg-gray-100', bucket: null },
        ] as const).map(({ label, value, colour, icon, bgObj, bucket }) => (
          <div key={label}
            onClick={() => {
              if (!bucket) return;
            ...
            className={`bg-white p-5 rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] border border-gray-100 flex flex-col justify-between min-h-[120px] transition-all text-left ${bucket ? 'cursor-pointer hover:shadow-[0_4px_15px_-3px_rgba(0,0,0,0.1)] hover:border-blue-300' : ''}`}>
             ...
             <div className="mt-auto">
                <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">{label}</p>
                <p className={`text-2xl font-extrabold leading-none ${colour}`}>{value}</p>
             </div>
          </div>
        ))}
      </div>
      ...
      {/* Controls */}
      <div className="bg-white rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] border border-gray-100 p-4 space-y-4">
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          {(userProfile?.role === 'operator' || isAdmin) && (
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => { setNewTemplate(emptyTemplate(companyId, user?.id ?? '')); setShowTplAddModal(true); }} disabled={actionLoading}
      ...
      {viewTab === 'templates' && (
        templatesLoading ? (
          ...
          <div className="bg-white rounded-xl border p-12 text-center">
            ...
            {(userProfile?.role === 'operator' || isAdmin) && (
              <button onClick={() => { setNewTemplate(emptyTemplate(companyId, user?.id ?? '')); setShowTplAddModal(true); }}
