...
import { AlertCircle, Calendar, MapPin, PlusCircle, Repeat } from 'lucide-react';
...
interface BranchDetailPanelProps {
  ...
  onAddRoute: () => void;
  onAddSchedule: () => void;
  onAddReturnSchedule: (reverseRouteId?: string) => void;
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
      <div className="p-5 border-b border-gray-100 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between bg-gray-50/50">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() => onAddReturnSchedule(selectedRoute?.id)}
            disabled={!selectedRoute}
            className="text-xs font-semibold text-white bg-slate-700 hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed px-3 py-1.5 rounded-lg flex items-center gap-1"
          >
            <Repeat className="w-3 h-3" /> Create Return Schedule
          </button>
        </div>
      </div>
